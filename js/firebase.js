...
async function backupProfile(profileId) {
  ...
  const fields = ['xp', 'coins', 'gamesPlayed', 'goodGamesStreak', 'records', 'badges',
    ...
    'age', 'catLevel', 'catStreak', 'errorBank'];
    // SECURITY: 'recoveryCode' is intentionally NOT backed up. The backup node
    // (players/{uid}/backup) is readable by any authenticated user, and the
    // recovery code is a secret bearer token. The recovery flow uses the
    // recovery/{code} -> uid index, not the backup blob, so the code is not
    // needed here. A restored profile regenerates its code via MQSync.
...
async function restoreFromCode(code) {
  ...
  const fields = ['xp', 'coins', 'gamesPlayed', 'goodGamesStreak', 'records', 'badges',
    ...
    'freeHints', 'shields', 'defeatedBosses', 'contractsCompleted', 'weeklyXP', 'weeklyGames', 'bossTitles', 'activeTitle',
    'age', 'catLevel', 'catStreak', 'errorBank'];
  // Note: 'recoveryCode' is no longer in the backup (see backupProfile).
  // A restored profile gets a fresh code generated by MQSync on next launch.
